...
def greedy_actions_pool(values, pool, num=1):
    values = values[pool].view(-1)
    rank = torch.argsort(values, descending=True)[:num]
    return rank, pool[rank], values[rank]

def uniform_actions_pool(values, pool, num=1):
    values = values[pool].view(-1)
    n = values.shape[0]
    sample_prob = torch.ones(n) / n
    random_sample = torch.multinomial(sample_prob, num_samples=num)
    return random_sample, pool[random_sample], values[random_sample]
...
def accuracy(y_pred, labels):
    ...
    acc = torch.sum((predicted == labels)) * 1.0 / predicted.size(0)
    return acc.item()
